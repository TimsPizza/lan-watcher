...
class NetworkScanner:
    ...
    async def get_local_subnet(self) -> str:
        ...
        if (
            self.config
            and self.config.subnet_cidr
            and not self.config.auto_detect_subnet
        ):
        ...
        try:
            ...
            for line in lines:
                if "UP" in line and (
                    "192.168." in line or "10." in line or "172." in line
                ):
                    # 提取网络地址
                    parts = line.split()
                    for part in parts:
                        if "/" in part and any(
                            net in part for net in ["192.168.", "10.", "172."]
                        ):
                            return part
        except Exception:
            pass
    ...
    async def ping_host(self, ip: str) -> Optional[DeviceInfo]:
        ...
        try:
            ...
            if devices:
                device = devices[0]
                # 根据配置补充主机名和厂商信息
                if not device.hostname and (
                    not self.config or self.config.resolve_hostnames
                ):
                    device.hostname = await self._get_hostname(device.ip)
                if (
                    device.mac
                    and not device.vendor
                    and (not self.config or self.config.fetch_vendor_info)
                ):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                device = DeviceInfo(ip=ip, is_online=True, response_time=response_time)
                ...
                for line in result.stdout.split("\n"):
                    ...
                    if len(parts) >= 2:
                        ...
                        if self._is_valid_ip(ip):
                            ...
                            devices.append(
                                DeviceInfo(
                                    ip=ip, mac=mac, vendor=vendor, is_online=True
                                )
                            )
    ...
    async def scan_ports(
        self, targets: List[str], ports: str = "1-1000"
    ) -> List[DeviceInfo]:
        """端口扫描功能"""
        if not targets:
            return []
    ...
    async def comprehensive_scan(self, subnet: str) -> List[DeviceInfo]:
        ...
        try:
            ...
            for i in range(0, len(active_ips), batch_size):
                ...
                batch_results = await self.scan_ports(
                    batch, "22,23,53,80,135,139,443,445,993,995"
                )
    ...
    async def scan_subnet(
        self, subnet: str, scan_type: str = "ping"
    ) -> List[DeviceInfo]:
        """扫描整个子网"""
        devices = []
        ...
        try:
            if scan_type == "arp" or scan_type == "ping":
                # 使用配置生成nmap参数
                if self.config:
                    nmap_args = self.config.get_nmap_args(subnet)
                    # 确保输出为XML格式
                    if "-oX" not in nmap_args and "-oN" not in nmap_args:
                        nmap_args.extend(["-oX", "-"])
                        
                    print(f"使用nmap扫描子网 {subnet}..., args: {nmap_args}")
                    ...
                    print(f"使用默认配置扫描子网 {subnet}..., args: {nmap_args}")
                ...
                # 补充信息：主机名、MAC地址和厂商（根据配置决定）
                filtered_devices = []
                for device in devices:
                    if device.is_online:
                        # 检查是否应该排除该IP
                        if self.config and self.config.should_exclude_ip(device.ip):
                            continue
            ...
            if scan_type == "arp":
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
                    ...
                    for ip in network.hosts():
                        task = loop.run_in_executor(executor, self._sync_ping, str(ip))
                        tasks.append(task)

                    results = await asyncio.gather(*tasks)
                    devices = [d for d in results if d is not None]

            # 为在线设备尝试获取主机名和厂商信息（根据配置决定）
            filtered_devices = []
            for device in devices:
                if device.is_online:
                    # 检查是否应该排除该IP
    ...
    def _sync_ping(self, ip: str) -> Optional[DeviceInfo]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return DeviceInfo(ip=ip, is_online=True, response_time=response_time)
        except Exception:
            pass
